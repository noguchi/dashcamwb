...
`calibrate` / `render <event_dir>` / `verify <event_dir>` / `render-all --source <dir>` / `serve` / `prune-recent` / `highlight-day` / `sync-insta360`。
...
`sync-insta360` は Insta360 の乗車視点 `.insv` と Tesla ドラレコ front クリップを時間同期し、(B) 横並び＋telemetry 焼き込み合成 mp4 と (C) `serve` の同期プレイヤー（手動ナッジ）を出す。同期はハイブリッド: ① `.insv` の `creation_time`(UTC) と front ファイル名(JST) で粗オフセット（タイムスタンプ・アンカー）→ ② 共通モーション信号の相互相関で精密化（Tesla=GPS course 由来ヨーレート、Insta360=トレーラの IMU ジャイロ）→ ③ プレイヤー上の手動ナッジで確定。**実機検証メモ: 当ファームの Tesla SEI は `heading_deg` が未 populate（全ゼロ）のため Tesla 側ヨーは GPS course から導出する。それでも相互相関は弱く（peak≈0.1）、②は信頼度 < 0.35 のときタイムスタンプ・アンカーへフォールバックする（ノイズで悪化させない）。実用上は ①アンカー（カメラ内蔵時計精度で±数秒）＋③手動ナッジで「完全同期」に到達する設計。** Insta360 IMU は独自トレーラ（末尾、offsets-index 版・20バイト raw int16・1000Hz）を `seek` して読む（18GB 本体は読まない、`insta360.read_imu`）。表示用乗車視点は `--insta-flat` の書き出し、無指定なら ffmpeg `v360` で dfisheye→平面に自動リフレーム。出力は `--out-root`(既定 `sync-work`)/`sync/<date>/` に `combined-<date>.mp4`・`sync.json`(δ・信頼度・telemetry・パス)・抜粋。モジュール: `insta360.py`(creation_time＋トレーラ IMU パーサ、vendored `vendor/insta360/NOTICE`)、`sync.py`(信号処理・アンカー・相関・ASS字幕・manifest)、`telemetry.iter_segment_frames`(per-frame SEI)、`ffmpeg_wrap.render_sidebyside`/`reframe_insv`、`serve` の `/sync/<date>` プレイヤー＋`/sync-nudge`。詳細は `docs/superpowers/specs/2026-05-30-insta360-tesla-sync-design.md`。
